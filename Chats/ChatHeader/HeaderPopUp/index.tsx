            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 50 }}>
                ...
                { !convo.isHighlight && !convo.isDiscoverable && <View>
                    ...
                    { !keepingUp && <TouchableOpacity onPress={handleKeepUp} style={ keepingUp ? styles.dropConversation : styles.keepUpWithConversation}>
                        <Text style={[styles.keepUpWithConversationText, { color: 'white' }]}>Keep Up With This Conversation</Text>
                    </TouchableOpacity>}
                </View>}
                ...
                {Platform.OS === 'android' && 
                    <View style={styles.androidMediaContainer}>
                        {convo.files.map((file:any, index:any) => (
                            <View>
                                { (file.endsWith('.mp4') || file.endsWith('.avi') || file.endsWith('.mov')) ?
                                 (<TouchableOpacity onPress={() => handleShowFullScreen(file)} style={styles.androidMediaButton}>
                                    <View style={styles.playButtonImageContainer}>
                                        <Image style={styles.playButtonImage} source={require('@/assets/images/play.png')}/>
                                    </View>
                                    <RemoteVideo resizeMode={ResizeMode.COVER} shouldPlay={false} path={ file } key={index} style={styles.androidMedia}/> 
                                </TouchableOpacity>) : (
                                    <TouchableOpacity onPress={() => handleShowFullScreen(file)} style={styles.androidMediaButton}>
                                        <RemoteImage path={ file } key={index} style={styles.androidMedia}/>
                                    </TouchableOpacity>
                                ) }
                            </View>
                        ))}
