...
import { chatType, convoType } from '@/types'
...
const Convo = (convo: convoType) => {
  ...
  const rightSwipe = useCallback(() => {
    return (
      <>
        {authenticatedUserData?.user_id === convo.user_id && Platform.OS === 'ios' && (
          <TouchableOpacity
  ...
  const pauseAudioIfNeeded = useCallback(() => {
    safePausePlayer()
...
useEffect(() => {
  const channelName = `convo-usersEngaged-channel-${convo.convo_id}-${Date.now()}`
        ...
        >
          ...
          >
            <RemoteVideo
              ...
              style={
                single
                  ? [styles.image, { width: Dimensions.get('window').width - 20 }]
                  : styles.image
              }
            />
          </VisibilityAwareView>
  ...
  return (
    <GestureHandlerRootView>
      <Swipeable renderRightActions={rightSwipe}>
        <VisibilityAwareView minVisibleArea={0.95} onBecomeInvisible={pauseAudioIfNeeded}>
          <TouchableOpacity
            activeOpacity={1}
          ...
          >
            <Animated.View style={[styles.popUpContainer, animatedPopupStyles]}>
              ...
              {Platform.OS === 'android' && convo.user_id === authenticatedUserData?.user_id && (
                <TouchableOpacity onPress={handleDelete} style={[styles.popUpOptionButton]}>
                  ...
                  >
                    {convo.Users && (
                      <RemoteImage
                        ...
                        path={`${convo.Users?.username}-profileImage`}
                      />
                    )}
