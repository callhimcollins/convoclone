...
const ChatHeader = () => {
  ...
  const fetchConvoAudio = useCallback(async () => {
    try {
      ...
      const { data } = supabase.storage.from('userfiles').getPublicUrl(String(convoData.audio))
  ...
  useEffect(() => {
    if (!playerStatus.playing) {
      ...
      if (finished) {
        ...
        dispatch(
          setAudioState({
            currentlyPlayingAudioID: String(convoData?.convo_id),
            isPaused: true,
          })
        )
      }
  ...
  const renderProfileImage = () => {
    const imagePath = convoData?.userData?.profileImage ?? convoData?.Users?.profileImage
  ...
  const renderFooter = () => (
    <View style={styles.footer}>
      ...
      <TouchableOpacity
        onLongPress={() => playPauseAudio('convo', convoAudio, String(convoData?.convo_id))}
    ...
    if (Platform.OS === 'android' || appearanceMode.name === 'light') {
      return (
        <View
          style={[
            styles.container,
            { elevation: 10, backgroundColor: appearanceMode.backgroundColor },
          ]}
        >
          {renderTop()}
          {renderFooter()}
