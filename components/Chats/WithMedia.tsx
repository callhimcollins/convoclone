...
import { addToInputState, setChatFiles, setReplyChat } from '@/state/features/chatSlice'
  ...
  const sendChat = async () => {
    ...
    if (!error) {
      ...
      dispatch(
        setSystemNotificationData({
          type: 'neutral',
          message: `Please wait while your ${files.length === 1 ? 'file' : 'files'} ${
      ...
      return (
        <KeyboardAvoidingView style={{ position: 'absolute', bottom: 0, width: '100%' }}>
          ...
          >
            ...
            <TouchableOpacity disabled={convoExists === false}>
              <Ionicons
                name="attach"
      ...
      <GestureDetector gesture={gesture}>
        <View style={{ flex: 1 }}>
          ...
          >
            ...
            <Text style={{ color: 'white', fontFamily: 'extrabold', fontSize: 20 }}>
              Send Files
            </Text>
          ...
          >
            ...
            >
              {files.map((file: any, index: number) => {
                const previewHeight = replyChat ? 610 : 700
                const isVideo = file.uri?.toLowerCase?.().endsWith('.mp4')
                  ...
                  <View key={`${file.uri}-${index}`}>
                    {isVideo ? (
                      <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                        ...
                        <LocalVideoPreview
                          uri={file.uri}
                          shouldPlay={videoPlaying.index === index && videoPlaying.playing}
