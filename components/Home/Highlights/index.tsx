...
import { pauseVideo, playVideo, togglePlayPause } from '@/state/features/mediaSlice'
...
const Highlights = ({ highLightUsers, highlight }: HighlightProps) => {
  ...
  const handleNextHighlight = () => {
    if (highLightUsers && Number(activeHighlight) >= highLightUsers.length - 1) {
      visibleInactiveValue.value = withTiming(1)
      visibleActiveValue.value = withTiming(1)
  ...
  const handleKeyPressAction = () => {
    ...
    if (
      ...
      ['mp4', 'mov', 'avi'].some(
        (ext) => highlight.Convos.files && String(highlight.Convos.files[0]).endsWith(ext)
      )
    ) {
      if (input !== '') {
  ...
  const renderMedia = () => {
    ...
    const isImage = ['jpg', 'png', 'jpeg', 'webp'].some((ext) => String(file).endsWith(ext))
