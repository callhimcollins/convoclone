...
const LocalVideoPreview = ({ uri, style }: { uri: string; style: any }) => {
...
const CreateDiscoverPost = () => {
  ...
  const sendNotification = async () => {
    try {
      ...
      if (data) {
        notify('neutral', 'Sending Notifications For Highlights...')
  ...
  const handleCreatePost = async () => {
    ...
    if (data) {
      ...
      if (!updateError) {
        const { error } = await supabase.from('discoverables').insert({
          title,
          convo_id: data.convo_id,
        })

        if (!error) {
          notify('success', 'Discover Post Created')
    ...
    <KeyboardAwareScrollView style={styles.container}>
      ...
      <View
        style={[
          ...
          file.length > 0 && {
            borderStyle: 'solid',
            borderColor: appearanceMode.backgroundColor,
          },
        ]}
      >
        {file.length === 0 && (
          <TouchableOpacity onPress={pickFile} style={styles.mediaButton}>
            <Feather size={100} color={appearanceMode.textColor} name="image" />
          </TouchableOpacity>
        )}

        {file.length === 1 && file[0].type === 'image' && (
          <TouchableOpacity onPress={pickFile} style={styles.mediaButton}>
            <Image source={{ uri: file[0].uri }} style={styles.mediaImage} />
          </TouchableOpacity>
        )}

        {file.length === 1 && file[0].type === 'video' && (
          <TouchableOpacity onPress={pickFile} style={styles.mediaButton}>
            <LocalVideoPreview uri={file[0].uri} style={styles.mediaImage} />
          </TouchableOpacity>
        )}
      </View>
      ...
      <View style={styles.inputContainer}>
        ...
        <TextInput value={url} onChangeText={setUrl} style={styles.textInput} placeholder="Link" />
      </View>
